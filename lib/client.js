window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region settings
		const STORAGE_KEY = "dsh-collapse-history:settings:v2";
		...
		const T = {
			...
			keepUser: isZh ? "保留最近你的消息条数" : "Keep recent of your messages",
			clearPins: isZh ? "清除手动固定（恢复自动折叠）" : "Clear manual pins (resume auto fold)",
			reset: isZh ? "恢复默认设置" : "Reset defaults",
			hint: isZh
				? "自动模式下，较旧的助手回复与你的消息会整段收起，最近 N 轮保持展开；手动操作过的轮次会被固定，直到清除固定。"
				: "In auto mode, older assistant replies and your messages fold entirely; the most recent N turns stay expanded. Manually toggled turns are pinned until you clear pins."
		};
		//#endregion
		...
		function autoFold() {
			...
			users.forEach((turn, index) => {
				if (index >= users.length - keepUser) setTurnHidden(turn, false);
				else setTurnHidden(turn, true);
			});
		}
		//#endregion
